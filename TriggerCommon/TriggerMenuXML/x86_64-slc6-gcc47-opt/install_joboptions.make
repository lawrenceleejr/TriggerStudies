...
#cmt_local_tagfile_install_joboptions = $(TriggerMenuXML_tag)_install_joboptions.make
...
#cmt_local_tagfile_install_joboptions = $(TriggerMenuXML_tag).make
...
#cmt_final_setup_install_joboptions = $(bin)TriggerMenuXML_install_joboptionssetup.make
...
#cmt_final_setup_install_joboptions = $(bin)TriggerMenuXMLsetup.make
cmt_local_install_joboptions_makefile = $(bin)install_joboptions.make
...
install_dir = ${installarea}/jobOptions/TriggerMenuXML
...
runHLT_forXMLgeneration_py_dependencies = ../share/runHLT_forXMLgeneration.py


install_joboptionsinstall :: ${install_dir}/runHLT_forXMLgeneration.py ;

${install_dir}/runHLT_forXMLgeneration.py :: ../share/runHLT_forXMLgeneration.py
	@if test ! "${installarea}" = ""; then \
	  d=`dirname ../share/runHLT_forXMLgeneration.py`; \
	  d=`(cd $${d}; pwd)`; \
	  CMTINSTALLAREA=${CMTINSTALLAREA}; export CMTINSTALLAREA; \
	  $(cmt_install_action) "$${d}" "runHLT_forXMLgeneration.py" "$(install_dir)" "/afs/cern.ch/atlas/software/builds/nightlies/devval/AtlasCore/rel_5/External/ExternalPolicy/src/symlink.sh" "$($(package)_cmtpath)"; \
	fi

../share/runHLT_forXMLgeneration.py : ;
...
install_joboptionsuninstall ::
	@if test ! "${installarea}" = ""; then \
	  ...
	  d=`(cd $${d}; pwd)`; \
	  CMTINSTALLAREA=${CMTINSTALLAREA}; export CMTINSTALLAREA; \
	  $(cmt_uninstall_action) "$${d}" "runHLT_forXMLgeneration.py" "$(install_dir)" "$($(package)_cmtpath)"; \
	fi

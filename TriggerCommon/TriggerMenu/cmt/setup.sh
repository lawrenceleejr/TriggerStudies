...
${CMTROOT}/${CMTBIN}/cmt.exe setup -sh -pack=TriggerMenu -version=TriggerMenu-00-05-00 -path=/afs/cern.ch/user/l/leejr/work/NightlyTest/Trigger/TriggerCommon  -no_cleanup $* >${cmtTriggerMenutempfile}
if test $? != 0 ; then
  echo >&2 "${CMTROOT}/${CMTBIN}/cmt.exe setup -sh -pack=TriggerMenu -version=TriggerMenu-00-05-00 -path=/afs/cern.ch/user/l/leejr/work/NightlyTest/Trigger/TriggerCommon  -no_cleanup $* >${cmtTriggerMenutempfile}"
  cmtsetupstatus=2

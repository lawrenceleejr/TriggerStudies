...
class Alg(object):
    ...
    def __init__(self, factory, args, kargs, manual_attrs={}):
        """set up the factory function (usally the class name)
        used ot obtain an instance of the Athena Alg python class,
        and its __init__ arguments. The manual_attrs dictionary
        carries attribute name and values. The values will be
        passed to eval before setting them as an attribute."""
...
def topoClusterMaker_partial(menu_data):
    
    do_lc = menu_data.cluster_params.do_lc
...
def topoClusterMaker_fullcalo(menu_data):

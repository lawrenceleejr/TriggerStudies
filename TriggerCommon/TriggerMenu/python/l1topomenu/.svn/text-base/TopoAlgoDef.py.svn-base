...
M7_menu_hack=True
_etamax = 31 if M7_menu_hack else 49
...
class TopoAlgoDef:
    ...
    def registerTopoAlgos(tm):
        ...
        for jet_type in ['AJ', 'CJ', 'J']:
            ...
            alg.addgeneric('jet_size', HW.DefaultJetSize.value) # JetSize                       
            #alg.addvariable('threshold', 0)  
            ...
            alg.addgeneric('jet_size', HW.DefaultJetSize.value)
            alg.addvariable('threshold', 0)  
            alg.addvariable('MinEta', 0)
            alg.addvariable('MaxEta', jetabseta)
            tm.registerAlgo(alg) # OK
        ...
        alg.addgeneric('OutputWidth', HW.OutputWidthSortJET )
        alg.addgeneric('jet_size', 1 if HW.DefaultJetSize.value==2 else 2)                
        #alg.addvariable('threshold', 0)
        alg.addvariable('MinEta', 0)
        ...
        alg.addgeneric('OutputWidth', HW.OutputWidthSortEM)
        #alg.addvariable('threshold', 0)
        alg.addvariable('IsoMask', 3) # Need to confirm 
        alg.addvariable('MinEta', 0)
        alg.addvariable('MaxEta', _etamax)
        tm.registerAlgo(alg)
        ...
        #alg.addvariable('threshold', 0)
        alg.addvariable('IsoMask', '00001') # Need to confirm
        alg.addvariable('MinEta', 0)
        alg.addvariable('MaxEta', _etamax)
        tm.registerAlgo(alg)
        ...
        alg = AlgConf.METSortingAlg( name = 'XE', inputs = 'MetIn', outputs = 'XE', algoId = currentAlgoId ); currentAlgoId += 1
        alg.addvariable('MinEta', 0)
        ...
        alg.addvariable('threshold', 0) # --> MinEt
        alg.addvariable('IsoMask', 0)                
        alg.addvariable('MinEta', 0)
        alg.addvariable('MaxEta', _etamax)
        tm.registerAlgo(alg) # OK
        ...
        alg.addgeneric('OutputWidth', HW.OutputWidthSelectEM)
        alg.addvariable('threshold', 0) # --> MinEt
        alg.addvariable('IsoMask', '00003') # Need to confirm with the simulation side for 'hi'
        alg.addvariable('MinEta', 0)
        alg.addvariable('MaxEta', _etamax)
        tm.registerAlgo(alg) # OK
        ...
        alg = AlgConf.MuonAbbreviated( name = 'MUab', inputs = 'MuonIn', outputs = 'MUab', algoId = currentAlgoId ); currentAlgoId += 1
        alg.addgeneric('NumberOfMuons', HW.OutputWidthSelectMU)
        alg.addvariable('MinEta', 0)
        alg.addvariable('MaxEta', 25)
        tm.registerAlgo(alg)

        alg = AlgConf.MuonAbbreviated( name = 'CMUab', inputs = 'MuonIn', outputs = 'CMUab', algoId = currentAlgoId ); currentAlgoId += 1
        alg.addgeneric('NumberOfMuons', HW.OutputWidthSelectMU)
        alg.addvariable('MinEta', 0)
        ...
        # All lists:
        alg = AlgConf.GenericListGenerator( name = 'EMall', inputs = 'ClusterTobArray', outputs = 'EMall', algoId = currentAlgoId) ; currentAlgoId += 1
        alg.addgeneric('JetSize', HW.DefaultJetSize)        
        tm.registerAlgo(alg)

        alg = AlgConf.GenericListGenerator( name = 'TAUall', inputs = 'ClusterTobArray', outputs = 'TAUall', algoId = currentAlgoId) ; currentAlgoId += 1
        alg.addgeneric('JetSize', HW.DefaultJetSize)        
        tm.registerAlgo(alg)

        alg = AlgConf.GenericListGenerator( name = 'AJall', inputs = 'JetTobArray', outputs = 'AJall', algoId = currentAlgoId ) ; currentAlgoId += 1
        alg.addgeneric('JetSize', HW.DefaultJetSize)        
        tm.registerAlgo(alg)

        alg = AlgConf.GenericListGenerator( name = 'AJjall', inputs = 'JetTobArray', outputs = 'AJjall', algoId = currentAlgoId ) ; currentAlgoId += 1
        alg.addgeneric('JetSize', 1 if HW.DefaultJetSize.value==2 else 2)
        tm.registerAlgo(alg)

        alg = AlgConf.GenericListGenerator( name = 'MUall', inputs = 'MuonIn', outputs = 'MUall',algoId = currentAlgoId) ; currentAlgoId += 1
        alg.addgeneric('JetSize', HW.DefaultJetSize)
        tm.registerAlgo(alg)
        ...
        for x in [     
            ...
            #                                            otype1, str(ocut1) if ocut1 > -1 else "", olist, str(nleading1) if olist=="s" else "",
            #                                            otype2, str(ocut2) if ocut2 > -1 else "", olist, str(nleading2) if olist=="s" else "")
        ...
        for x in [     
            {"minHT": 0,   "otype" : "AJ", "ocut" : 0,  "olist" : "s",   "nleading" : 5, "oeta" : _etamax},
            ...
            {"minHT": 200, "otype" : "AJ", "ocut" : 20, "olist" : "all", "nleading" : HW.InputWidthJET, "oeta" : 49},
            {"minHT": 190, "otype" : "AJ", "ocut" : 20, "olist" : "all", "nleading" : HW.InputWidthJET, "oeta" : 49},
            {"minHT": 150, "otype" : "AJj","ocut" : 15, "olist" : "all", "nleading" : HW.InputWidthJET, "oeta" : 49},
            {"minHT": 200, "otype" : "AJ", "ocut" : 20, "olist" : "s",   "nleading" : 5, "oeta" : 49},
            {"minHT": 190, "otype" : "AJ", "ocut" : 20, "olist" : "s",   "nleading" : 5, "oeta" : 49},
            ]:
            ...
            print toponame
            if M7_menu_hack and toponame =='HT0-AJ0s5.ETA31': toponame = "HT%d_%s%s" % (minHT, otype, olist)
            log.info("Define %s" % toponame)
            ...
            if otype.find('j')>=0:
                jetsize = (1 if HW.DefaultJetSize.value==2 else 2) 
            ...
            #alg.addgeneric('jet_size', 1 if HW.DefaultJetSize.value==2 else 2) # for M7
            #alg.addvariable('MinEta', 0)
            #alg.addvariable('MaxEta', oeta)
            #alg.addvariable('threshold', ocut)                        

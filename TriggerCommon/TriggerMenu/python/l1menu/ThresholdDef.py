...
class ThresholdDef:
    ...
    def registerThresholds(tc):
        ...
        #    tc.registerThr('JF%i.%iETA49' % (thrV, etamin), 'JET').addThrValue(JetOff).addThrValue( thrV, etamin=etamin, priority=2)
        #    tc.registerThr('JB%i.%iETA49' % (thrV, etamin), 'JET').addThrValue(JetOff).addThrValue( thrV, etamax=-etamin, priority=2)
        ...
        # Custom Forward jet
        for (thrV, etamin, etamax) in [ (15,23,49) ]:
        ...
        # XE
        for thrV in [35, 40, 45, 50, 55, 60, 70, 80, 90]:
